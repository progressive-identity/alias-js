...
use std::*;
use crate::seekable_bufreader::SeekBufReader;
use wasm_bindgen::prelude::*;

#[wasm_bindgen]
pub struct TarGzArchiveReader {
    js_file: Option<JsFile>,
    inner: Option<TarIterator<flate2::read::GzDecoder<ProbeReader<SeekBufReader<File>>>>>,
    watchers: Watchers,
    probe: ReadProbe,
    pass_count: u32,
}
...
impl TarGzArchiveReader {
    ...
    fn init_inner(&mut self) {
        ...
        let file = SeekBufReader::with_capacity(64 * 1024 * 1024, file);
...
impl ZipArchiveReader {
    #[wasm_bindgen(constructor)]
    pub fn new(js_file: JsFile) -> ZipArchiveReader {
        let mut archive = ZipArchiveReader {
            js_file: Some(js_file),
            inner: None,
            watchers: Watchers::new(),
            probe: ReadProbe::new(),
    ...
    fn init_inner(&mut self) {
        ...
        let file = SeekBufReader::with_capacity(64 * 1024 * 1024, file);
        let file = ProbeReader::new(file, &self.probe);
        let inner = ZipIterator::new(file);
        self.inner = Some(inner);
    }
